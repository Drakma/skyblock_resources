...
public class WoodenEmptyGeneratorOnBlockRightClickedProcedure {
		...
		if ((new Object() {
				...
				if (world instanceof Level _lvl) {
					...
					for (SetGeneratorGUIRecipe recipe : recipes) {
						...
						if (!ingredients.get(0).test((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY)))
							continue;
						if (!ingredients.get(1).test(new ItemStack(SkyblockResourcesModBlocks.WOODEN_EMPTY_GENERATOR.get())))
							continue;
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.anvil.place")), SoundSource.NEUTRAL, 1, (float) 0.1);
				} else {
					_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.anvil.place")), SoundSource.NEUTRAL, 1, (float) 0.1, false);
				}
			}
